...
/**
 * Static class that provides functions for loading AMR graphs or additional
 * information for AMR graphs from a file.
 */
public class AmrParser {
    private static final Pattern NAMED_VERTEX_PATTERN =
        Pattern.compile("\\(([^)]*?) / (.*?)[\\)\\s:]");
    private static final Pattern UNNAMED_NUMBER_PATTERN =
        Pattern.compile(":[^ ]+ ([0-9,\\.\\-\\+]+)(~e.[0-9,]+)?[ )]");
    private static final Pattern UNNAMED_WORD_PATTERN =
        Pattern.compile(":[^ ]+ (\"[^\"]+\")(~e.[0-9,]+)?[ )]");
    private static final Pattern UNNAMED_WORD_ALTERNATIVE_PATTERN =
        Pattern.compile(":[^ ]+ ([A-z][A-z][A-z][A-z]+)(~e.[0-9,]+)?[ )]");
    private static final Pattern ALIGNMENT_PATTERN =
        Pattern.compile("(.*?)~e\\.([0-9,]+)");
    ...
    /**
     * Adds POS tags to each reference sentence contained within a list of AMR
     * graphs.
     * @param amrs the list of AMR graphs
     * @param posTagFile the file in which the POS tags are stored. POS tags
     * must be tab separated with one sentence per line and the order of the
     * sentences must be the same as in the list {@code amrs}.
     */
    public static void addPosTags(List<Amr> amrs, String posTagFile)
        throws IOException {
        try (BufferedReader br =
                 new BufferedReader(new FileReader(posTagFile))) {
            int index = 0;
    ...
    /**
     * Adds the corresponding dependency tree to each reference sentence
     * contained within a list of AMR graphs.
     * @param amrs the list of AMR graphs
     * @param dependencyTreeFile the file in which the dependency trees are
     * stored in CoNLL format.
     */
    public static void addDependencyTrees(
        List<Amr> amrs, String dependencyTreeFile) throws IOException {
        DependencyTreeParser parser = new DependencyTreeParser();

        try (BufferedReader br =
                 new BufferedReader(new FileReader(dependencyTreeFile))) {
            int index = 0;
            ...
            for (String line; (line = br.readLine()) != null;) {
                ...
                if (!line.isEmpty()) {
                    ...
                    if (!edgeSpec.isEmpty()) {
                        amrs.get(index).dependencyTree = parser.fromString(
                            edgeSpec, amrs.get(index).sentence);
                        amrs.get(index).dependencyTree.amr = amrs.get(index);
                        edgeSpec.clear();
                        do {
                            index++;
                        } while (index < amrs.size()
                            && String.join(" ", amrs.get(index).sentence)
                                   .equals("."));
                    }
                }
            }
     ...
     * stored, see {@link AmrParser#addPosTags(List, String)}
     * @param limit the maximum number of AMR graphs to load. Set this to some
     * value &lt; 0 to load all AMR graphs found in the file.
     * @param format the format in which alignments are stored, see {@link
     * AmrLineFormat}
     * @return the list of AMR graphs extracted from {@code file}, along with
     * the dependency trees and POS tags of each reference sentence
     */
    public static List<Amr> fromFile(String file, String dependencyTreeFile,
        String posTagFile, int limit, AmrLineFormat format) throws IOException {
            ...
            for (String line; (line = br.readLine()) != null;) {
                ...
                if (collectingDagData) {
                    if (!line.trim().isEmpty()) {
                        ...
                        Amr amr = fromString(
                            dagReprBuilder.toString(), currentSentence);
     ...
     * @param format the format in which the alignments are represented, see
     * {@link AmrLineFormat}
     */
    public static void addAlignmentsFromFile(List<Amr> amrs, String filePath,
        AmrLineFormat format) throws IOException {
        for (Amr amr : amrs) {
    ...
    private static void addAlignmentFromLine(
        Amr amr, String line, AmrLineFormat format) {
    ...
    private static void addAlignmentFromString(
        Amr amr, String alignment, AmrLineFormat format) {
        String[] comps =
            alignment.split(AmrLineFormat.getWordNodeSeparator(format));
        ...
        if (format == AmrLineFormat.JAMR) {
            for (int i = Integer.valueOf(span[0]); i < Integer.valueOf(span[1]);
                 i++) {
                wordIds.add(i);
            }
    ...
    private static void addAlignmentFromEdgeIdPair(
        Amr amr, String edgeString, int id, AmrLineFormat format) {
    ...
    private static Edge getEdgeFromString(
        Amr amr, String edgeString, AmrLineFormat format) {
        List<Integer> edgeLevels;
        boolean relationEdge = false;
        switch (format) {
            case JAMR:
                edgeLevels = Arrays.asList(edgeString.split("\\."))
                                 .stream()
            ...
            case EXTERNAL:
                if (edgeString.charAt(edgeString.length() - 1) == 'r') {
                    relationEdge = true;
                    edgeString =
                        edgeString.substring(0, edgeString.length() - 2);
                }
                edgeLevels = Arrays.asList(edgeString.split("\\."))
                                 .stream()
                                 .map(s -> Integer.valueOf(s))
        ...
        while (!edgeLevels.isEmpty()) {
            ...
            switch (format) {
                case EXTERNAL:
                    relevantEdges =
                        currentVertex
                            .getOutgoingEdges(); // eigentlich müsste der
                                                 // InstanceEdge entfernt werden
                                                 // UND nextChildId-=1 gerechnet
                                                 // werden, aber beides hebt
                                                 // sich gegenseitig auf!
                    break;
                case JAMR:
                    relevantEdges = currentVertex.getOutgoingEdges()
                                        .stream()
                                        .filter(e
                                            -> !e.debugInfo.equals("link")
                                                && !e.isInstanceEdge())
                                        .collect(Collectors.toList());
                    break;
                default:
    ...
    /**
     * Builds an AMR graph from a string representation of the graph in
     * LDC2014T12 format and assigns a reference sentence to it.
    ...
    public static Amr fromString(String dagRepr, String[] sentence) {
        ...
        dagRepr =
            getVertices(dagRepr, result.dag, vertexMap, idMap, unnamedVertices);
        vertexMap.put(Vertex.EMPTY_VERTEX_ID, Vertex.EMPTY_VERTEX);
        for (Vertex v : vertexMap.values()) {
            if (!unnamedVertices.contains(v)) {
                dagRepr = dagRepr.replace(
                    idMap.get(v) + " / " + v.instance, idMap.get(v) + "@L@");
            } else {
            }
    ...
    private static void generateEdges(
        String dagRepr, Map<String, Vertex> vertexMap, DirectedGraph dag) {
        String[] split;
                ...
                for (int i = 0; i < dagRepr.length(); i++) {
                    ...
                    if (currentChar == '(' && !inQuotes) {
                        ...
                        if (level == 0) {
                            endIndex = i;
                            generateEdges(
                                dagRepr.substring(startIndex, endIndex + 1),
                                vertexMap, dag);
                            break;
                        }
                    }
    ...
    private static String getVertices(String dagRepr, DirectedGraph dag,
        Map<String, Vertex> vertexMap, Map<Vertex, String> idMap,
        Set<Vertex> unnamedVertices) {
        vertexMap.clear();
        idMap.clear();
        ...
        for (Pattern p : Arrays.asList(UNNAMED_NUMBER_PATTERN,
                 UNNAMED_WORD_PATTERN, UNNAMED_WORD_ALTERNATIVE_PATTERN)) {
