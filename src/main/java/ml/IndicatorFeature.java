...
abstract class IndicatorFeature {
    ...
    /**
     * Turns this feature into a context, i.e. a list of strings that can then
     * be turned into feature functions by a maximum entropy model.
     * @return the feature's representation as a list of string
     */
    abstract List<String> toContext();

    /**
     * Returns whether the feature can be composed with other features to form a
     * composite feature.
     ...
     * @return the composed feature or null if one of the features is not
     * composable as defined by {@link IndicatorFeature#isComposable()}
     */
    public IndicatorFeature composeWith(IndicatorFeature feature, String id) {
        if (!isComposable() || !feature.isComposable())
            return null;
        ...
        ListFeature ret =
            new ListFeature(this.getName() + "-" + feature.getName());
    ...
    /**
     * Returns whether the feature represents a list of elements (e.g. the set
     * of outgoing edge labels of some vertex) or just a single element (e.g.
     * the concept label of a vertex).
