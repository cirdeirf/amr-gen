 ...
 * methods. The features used by {@link NumberMaxentModel#toDatumList(Amr,
...
public class NumberMaxentModel extends StanfordMaxentModelImplementation {
    ...
    public List<Datum<String, String>> toDatumList(
        ...
        if (!forTesting) {
            if (!PosHelper.mapPos(vertex.getPos(), true).equals("NN"))
        ...
        features.add(new ListFeature("outLabel-posTag",
            outEdges.stream()
                .map(e
                    -> e.getLabel()
                        + (e.getTo().isPropbankEntry() ? ":PROP"
                                                       : e.getTo().getPos()))
                .collect(Collectors.toList())));
        features.add(new ListFeature("outLabel",
            outEdges.stream()
                .map(e -> e.getLabel())
                .collect(Collectors.toList())));
        features.add(new ListFeature("outLabel-instance",
            outEdges.stream()
                .map(e -> e.getLabel() + instance)
                .collect(Collectors.toList())));

        features.add(new StringFeature(
            "toDepthString-instance", depthString + instance));
        features.add(new StringFeature(
            "parentInst-instance", parentInstance + instance));
    ...
    public void applyModification(
        ...
        if (forcePlural(vertex))
            vertex.predictions.put("number",
                Collections.singletonList(
                    new Prediction(GoldSyntacticAnnotations.PLURAL, 1)));
        else
            vertex.predictions.put("number", predictions);
    }
    ...
    public static boolean forcePlural(Vertex v) {
        for (Edge e : v.getOutgoingEdges()) {
            ...
            if (e.getLabel().equals(":quant")
                && !e.getTo().getInstance().equals("1"))
                return true;
            if (e.getLabel().equals(":mod")
                && e.getTo().getInstance().equals("various"))
                return true;
            if (e.getLabel().equals(":domain")
                && e.getTo().getInstance().equals("they"))
                return true;
        }
