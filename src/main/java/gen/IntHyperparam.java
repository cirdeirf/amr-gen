...
/**
 * This class is used to represent a special kind of hyperparameter that allows
 * only integer values (see {@link Hyperparam} for more details).
 */
public class IntHyperparam extends Hyperparam {
     ...
     * @throws AssertionError if {@code defaultValue}, {@code min} or {@code
    ...
    public IntHyperparam(double defaultValue, double min, double max) {
        ...
        if (!isInteger(defaultValue) || !isInteger(min) || !isInteger(max)) {
            throw new AssertionError(
                "IntHyperparam values must be integers. Found (defaultValue = "
