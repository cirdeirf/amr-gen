...
public class FirstStageProcessor {
    ...
    private boolean isApplicable(Vertex current, String transition) {
        ...
        if (transition.equals(GoldTransitions.MERGE_SIBLING)) {
            // there cannot be a MERGE-SIBLING transition if there is no parent
            if (current.getIncomingEdges().isEmpty())
                return false;

            // gather parent and sibling vertices
            Vertex parent = current.getIncomingEdges().get(0).getFrom();
            ...
            siblingVertices.remove(current);

            // no MERGE-SIBLING transition without siblings
            if (siblingVertices.isEmpty())
                return false;

            List<String> siblingsToMerge = new ArrayList<>();
            // go through all siblings and check if one of them is mergeable
            // with the current vertex
            for (Vertex v : siblingVertices) {
                siblingsToMerge.clear();
                siblingsToMerge.add(v.getInstance());
                siblingsToMerge.add(current.getInstance());
                // sort the instances
                // (necessary since all merges in the map are sorted too)
                Collections.sort(
                    siblingsToMerge, String.CASE_INSENSITIVE_ORDER);

                // construct the possible key (the two instances to be merged)
                String key = "";
                ...
                // MERGE-SIBLING is only allowed if a merge between two siblings
                // has been observed during training, i.e. can be found in the
                // mapping generated by {@link
                // StaticHelper:getMergeSiblingMap(List)}
                if (maxentModel.bestSiblingMerges.containsKey(key))
                    return true;
            }
        }
    ...
    private boolean applyTransition(Vertex current, String transition) {
        ...
        switch (transition) {
            ...
            case GoldTransitions.MERGE:
                ...
                return false;
            case GoldTransitions.MERGE_SIBLING:
                ...
                // omit the current vertex itself
                siblingVertices.remove(current);

                // determine if at least one MERGE-SIBLING has been applied
                boolean mergedSibling = false;

                List<String> siblingsToMerge = new ArrayList<>();
                // go through all siblings and merge them with the current
                // vertex whenever possible
                for (Vertex v : siblingVertices) {
                    siblingsToMerge.clear();
                    siblingsToMerge.add(v.getInstance());
                    siblingsToMerge.add(current.getInstance());
                    ...
                    key = key.substring(0, key.length() - 1);
                    String bestSiblingMerge =
                        maxentModel.bestSiblingMerges.get(key);
                    if (bestSiblingMerge != null) {
                        String[] comps = bestSiblingMerge.split("\t");
                        String inst = comps[0];
                        String pos = comps[1];

                        // merge sibling v (slave) into current (master)
                        amr.merge(current, v, inst, pos);

                        // successful application of the transition
                        mergedSibling = true;

                        // in case only one MERGE-SIBLING per vertex is desired:
                        // break;
                    }
                }
