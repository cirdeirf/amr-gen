...
/**
 * This helper class provides, for each vertex, the default realizations
 * described in Section 5 (Implementation) of the thesis.
 */
public class DefaultRealizer {
    // a set of words for which the default realization is the empty word
    private static final Set DEFAULT_EMPTY_REALIZATION =
        new HashSet<>(Arrays.asList("person", "thing", "amr-unknown",
            "have-org-role", "contrast", "distance-quantity"));

    // a map containing some handwritten default realizations
    private static final Map<String, List<String>> DEFAULT_REALIZATIONS =
        new HashMap<>();

    static {
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "possible", Collections.singletonList("can"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "obligate-01", Collections.singletonList("must"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "+", Collections.singletonList("please"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "-", Arrays.asList("no", "not"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "cause-01", Arrays.asList("due to", "because", "for"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "contrast-01", Collections.singletonList("but"));
        DefaultRealizer.DEFAULT_REALIZATIONS.put(
            "resemble-01", Collections.singletonList("like"));
    }
     ...
     * @param v the vertex
     * @param syntacticAnnotation the syntactic annotation assigned to this
     * vertex
     * @return the default realizations of {@code v}
     */
    public List<String> getDefaultRealizations(
        Vertex v, Map<String, Prediction> syntacticAnnotation) {
        ...
        if (Arrays.asList("he", "she", "i", "you", "they", "we", "it")
                .contains(inst)) {
            if (!v.getIncomingEdges().isEmpty()) {
                if ((inst.equals("you") || inst.equals("we"))
                    && v.getIncomingEdges().get(0).getFrom().mode.equals(
                           "imperative")) {
                    return Collections.singletonList("");
                }
                if (v.getIncomingEdges().get(0).getLabel().equals(":poss")) {
                    return Collections.singletonList(
                        WordLists.possessivePronouns.get(inst));
                }
            }
            List<String> ret = new ArrayList<>();
            ret.add(WordLists.possessivePronouns.get(inst));
            if (WordLists.personalPronouns.containsKey(inst))
                ret.add(WordLists.personalPronouns.get(inst));
            ret.add(inst);
            return ret;
        }
        ...
        if (v.isLink()) {
            if (!v.getIncomingEdges().isEmpty()) {
                if (v.getIncomingEdges().get(0).getLabel().equals(":poss")) {
                    if (v.annotation.original.predictions.containsKey(
                            "number")) {
                        if (v.annotation.original.predictions.get("number")
                                .get(0)
                                .getValue()
                                .equals(GoldSyntacticAnnotations.SINGULAR)) {
                            return Arrays.asList(
                                WordLists.possessivePronouns.get("he"),
                                WordLists.possessivePronouns.get("it"),
                                WordLists.possessivePronouns.get("she"));
                        } else {
                            return Arrays.asList(
                                WordLists.possessivePronouns.get("they"));
                        }
                    }
                }
            }
        ...
        Map<POS, Integer> instPosCounts =
            wordNetHelper.getAllPOSTagsWithCount(inst, true);
        int maxCount = 0;
        ...
        if (mappedPos.equals("VBG") && v.isPropbankEntry()) {
            if (!instPosTags.contains(POS.VERB))
                return Collections.emptyList();
            ...
            clause.setVerbPhrase(instSpec);
            clause.setFeature(
                simplenlg.features.Feature.FORM, Form.PAST_PARTICIPLE);
            inst = realizer.realise(clause).getRealisation();
            return Collections.emptyList();
            ...
            if (syntacticAnnotation.containsKey("number")) {
                if (syntacticAnnotation.get("number").getValue().equals(
                        GoldSyntacticAnnotations.PLURAL)) {
            ...
            if (instPosTags.contains(POS.NOUN))
                candidates.add(inst);
            ...
            for (String candidate : candidates) {
                NPPhraseSpec instSpec =
                    phraseFactory.createNounPhrase(candidate);
                SPhraseSpec clause = phraseFactory.createClause();
                clause.setSubject(instSpec);
                instSpec.setFeature(
                    simplenlg.features.Feature.NUMBER, numberAgreement);
                String realization = realizer.realise(clause).getRealisation();

                if (wordNetHelper.getAllPOSTagsWithCount(realization, true)
                        .getOrDefault(POS.NOUN, 0)
                    > 1) {
                    ret.add(realization);
                }
            }
            ...
            if (!v.mode.isEmpty()) {
                clause.setFeature(
                    simplenlg.features.Feature.FORM, Form.IMPERATIVE);
                ...
                if (syntacticAnnotation.containsKey("tense")) {
                    switch (syntacticAnnotation.get("tense").getValue()) {
                        case GoldSyntacticAnnotations.PAST:
                            tense = Tense.PAST;
                            break;
                        case GoldSyntacticAnnotations.PRESENT:
                            tense = Tense.PRESENT;
                            break;
                        case GoldSyntacticAnnotations.NONE:
                            return Collections.singletonList(inst);
                        case GoldSyntacticAnnotations.FUTURE:
                            return Collections.singletonList(inst);
                    }
                }
                ...
                clause.setFeature(simplenlg.features.Feature.NUMBER,
                    NumberAgreement.SINGULAR);
                clause.setFeature(
                    simplenlg.features.Feature.PERSON, Person.THIRD);

                String realization1 = realizer.realise(clause).getRealisation();

                clause.setFeature(
                    simplenlg.features.Feature.NUMBER, NumberAgreement.PLURAL);
                // clause.setFeature(simplenlg.features.Feature.TENSE,
                // Tense.PRESENT);

                String realization2 = realizer.realise(clause).getRealisation();

                if (realization1.equals(realization2))
                    return Collections.singletonList(realization1);
            ...
            if (!instPosTags.contains(POS.ADJECTIVE)
                && !instPosTags.contains(POS.ADVERB))
                return Collections.emptyList();
            ...
            if (instPosTags.contains(POS.ADJECTIVE)
                ...
                if (Arrays.asList("fast", "hard", "very", "quite")
                        .contains(inst)) {
                    adverb = inst;
        ...
        if (inst.equals("1")) {
            return Arrays.asList("one", "1");
        }
        if (inst.equals("2")) {
            return Arrays.asList("two", "2");
        }
    ...
    /**
     * Makes an educated guess about whether a vertex represents a verb that is
     * in third person.
     * @param amr the AMR graph to which the vertex belongs
     * @param vertex the vertex
     * @param forTesting whether this guess is made during test time (i.e.
     * whether no gold annotations are available)
     * @return true iff it is likely that the verb is in third person
     */
    public static boolean guessThirdPerson(
        ...
        for (int i = 0; i <= 1; i++) {
            for (Edge e : vertex.getOutgoingEdges()) {
                if (e.getLabel().equals(":ARG" + i)) {
                    if (WordLists.possessivePronouns.keySet().contains(
                            e.getTo().getInstance())) {
                        if (Arrays.asList("he", "she", "it")
                                .contains(e.getTo().getInstance()))
                            thirdPerson = true;
                    } else if (!forTesting) {
                        if (GoldSyntacticAnnotations
                                .getGoldNumber(amr, e.getTo())
                                .equals(GoldSyntacticAnnotations.SINGULAR))
                            thirdPerson = true;
                    } else {
                        if (e.getTo().predictions.containsKey("number")) {
                            if (e.getTo()
                                    .predictions.get("number")
                                    .get(0)
                                    .getValue()
                                    .equals(GoldSyntacticAnnotations.SINGULAR))
                                thirdPerson = true;
